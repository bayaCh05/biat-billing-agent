...
vi.mock('../api/endpoints', () => ({
  getReviewQueue: vi.fn().mockResolvedValue([{
    id: 'inv-001', status: 'FLAGGED', direction: 'SUPPLIER',
    issuer_name: 'OOREDOO TUNISIE', invoice_number: 'OOR-2026-001',
    invoice_date: '2026-06-01', amount_ht: 8000, tva_rate: 19,
    tva_amount: 1520, amount_ttc: 9520, currency: 'TND',
    accounting_compte: '6260', accounting_label: 'Télécoms',
    extraction_method: 'LLM',
    flags: [{ flag_type: 'TOTAL_MISMATCH', severity: 'ERROR', field_name: 'amount_ttc', message: 'Écart détecté', resolved: false }],
    human_review_required: true, has_errors: true,
    received_at: new Date().toISOString(),
  }]),
...
describe('ReviewQueue', () => {
  it('renders the queue item after fetch', async () => {
    render(<ReviewQueue />)
    await waitFor(() =>
  ...
  it('shows "File vide" when queue is empty after approve', async () => {
    render(<ReviewQueue />)
    await waitFor(() => screen.getByText('Approuver'))
    fireEvent.click(screen.getByText('Approuver'))
    await waitFor(() =>
