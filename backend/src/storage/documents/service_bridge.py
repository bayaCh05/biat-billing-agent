...
puis retournent None/False pour signaler le fallback SQLAlchemy.
Pattern utilisé pendant la période de coexistence (Phases 3–5).

Usage dans une route FastAPI :
    revoked = await is_token_revoked_mongo(jti)
    if revoked is None:
        # fallback SQLAlchemy
        revoked = db.get(RevokedTokenORM, jti) is not None
"""
...
async def is_token_revoked_mongo(jti: str) -> bool | None:
    """Vérifie si un JTI est révoqué dans MongoDB.

    Retourne :
        True  — token révoqué (trouvé dans MongoDB)
        False — token non révoqué (absent de MongoDB)
        None  — MongoDB non disponible (utiliser fallback SQLAlchemy)
    """
    try:
        from src.storage.documents.revoked_token import RevokedTokenDocument
        doc = await RevokedTokenDocument.get(jti)
        return doc is not None
    except Exception as exc:
        logger.debug("is_token_revoked_mongo: MongoDB indisponible — %s", exc)
        return None


async def revoke_token_mongo(
    jti: str,
    reason: str = "logout",
    user_id: str | None = None,
) -> bool:
    """Révoque un token dans MongoDB.

    Retourne True si l'opération a réussi, False si MongoDB est indisponible.
    Idempotent : ne crée pas de doublon si le JTI est déjà révoqué.
    """
    try:
        from src.storage.documents.revoked_token import RevokedTokenDocument
        existing = await RevokedTokenDocument.get(jti)
        if existing is None:
            doc = RevokedTokenDocument(id=jti, reason=reason, user_id=user_id)
            await doc.insert()
        return True
    except Exception as exc:
        logger.debug("revoke_token_mongo: MongoDB indisponible — %s", exc)
        return False


async def upsert_active_token_mongo(
    jti: str,
    user_id: str,
    expires_at,
    ...
    """Enregistre ou met à jour une session active dans MongoDB.

    Retourne True si l'opération a réussi, False si MongoDB est indisponible.
    """
    try:
        from src.storage.documents.active_token import ActiveTokenDocument
        doc = ActiveTokenDocument(
            id=jti,
            user_id=user_id,
        ...
        logger.debug("upsert_active_token_mongo: MongoDB indisponible — %s", exc)
        return False


async def mark_token_revoked_mongo(jti: str) -> bool:
    """Marque un token actif comme révoqué (soft delete) dans MongoDB.

    Retourne True si l'opération a réussi, False si MongoDB est indisponible.
    """
    try:
        from src.storage.documents.active_token import ActiveTokenDocument
        doc = await ActiveTokenDocument.get(jti)
        if doc is not None:
            doc.revoked = True
            await doc.save()
        return True
    except Exception as exc:
        logger.debug("mark_token_revoked_mongo: MongoDB indisponible — %s", exc)
        return False
...
def _phase_status_bridge(status: str, consumed_jh: float) -> str:
...
# ── Migration des écritures : miroir Mongo best-effort ───────────────────────
#
# Pattern pour chaque route d'écriture : SQLAlchemy reste la source de vérité
# (écrit et commit d'abord, doit réussir) ; le miroir Mongo est appliqué APRÈS,
# en best-effort — son échec ne fait jamais échouer la requête. Les _id et
# soft-refs sont des chaînes avec tirets, comme le script de migration
# (voir la note dans get_invoice_mongo() sur Document.insert() vs pymongo brut).
...
async def _mirror_upsert(doc_class, doc: dict) -> None:
    try:
        coll = doc_class.get_pymongo_collection()
        await coll.replace_one({"_id": doc["_id"]}, doc, upsert=True)
    except Exception as exc:
        logger.debug("_mirror_upsert(%s): miroir Mongo échoué — %s", doc_class.__name__, exc)


async def _mirror_delete(doc_class, doc_id: str) -> None:
    try:
        coll = doc_class.get_pymongo_collection()
        await coll.delete_one({"_id": doc_id})
    except Exception as exc:
        logger.debug("_mirror_delete(%s): miroir Mongo échoué — %s", doc_class.__name__, exc)


# ── Lot 2 écriture : roadmap / livrables / risks ─────────────────────────────

async def mirror_roadmap_item(item) -> None:
    from src.storage.documents.feuille_de_route import FeuilleDeRouteDocument

    await _mirror_upsert(FeuilleDeRouteDocument, {
        "_id": str(item.id),
        "titre": item.titre,
        "description": item.description,
        "date_debut": _to_midnight_utc(item.date_debut),
        "date_fin": _to_midnight_utc(item.date_fin),
...
async def mirror_delete_roadmap_item(item_id: str) -> None:
    """Supprime le jalon miroir + ses risques liés (réplique le ON DELETE CASCADE SQL)."""
    try:
        from src.storage.documents.feuille_de_route import FeuilleDeRouteDocument
        from src.storage.documents.risque import RisqueDocument
        ...
        logger.debug("mirror_delete_roadmap_item: miroir Mongo échoué — %s", exc)


async def mirror_livrable(lv) -> None:
    from src.storage.documents.livrable import LivrableDocument

    await _mirror_upsert(LivrableDocument, {
        "_id": str(lv.id),
        "phase_id": lv.phase_id,
        "titre": lv.titre,
        "description": lv.description,
        "date_livraison_prevue": _to_midnight_utc(lv.date_livraison_prevue),
        "date_livraison_reelle": _to_midnight_utc(lv.date_livraison_reelle),
        "statut": lv.statut,
        "fichier_path": lv.fichier_path,
        "created_by": lv.created_by,
    })


async def mirror_phase_status(phase) -> None:
    """Miroir du statut/closed_date d'une PhaseORM (utilisé par valider_phase)."""
    from src.storage.documents.phase import PhaseDocument
    ...
    try:
        ...
        await coll.update_one(
            ...
            {"$set": {
                "status": phase.status,
                "closed_date": _to_midnight_utc(phase.closed_date),
            }},
        )
    except Exception as exc:
        logger.debug("mirror_phase_status: miroir Mongo échoué — %s", exc)


async def mirror_risk(r) -> None:
    from src.storage.documents.risque import RisqueDocument

    await _mirror_upsert(RisqueDocument, {
        "_id": str(r.id),
        "titre": r.titre,
        ...
        "date_identification": _to_midnight_utc(r.date_identification),
        "date_echeance_mitigation": _to_midnight_utc(r.date_echeance_mitigation),
        "date_cloture": _to_midnight_utc(r.date_cloture),
...
# ── Lot 3 écriture : notifications / review (invoices) ───────────────────────

async def mirror_notification_read(notif_id: str) -> None:
    from src.storage.documents.notification import NotificationDocument

    try:
        coll = NotificationDocument.get_pymongo_collection()
        await coll.update_one({"_id": notif_id}, {"$set": {"is_read": True}})
    except Exception as exc:
        logger.debug("mirror_notification_read: miroir Mongo échoué — %s", exc)


async def mirror_mark_all_notifications_read() -> None:
    from src.storage.documents.notification import NotificationDocument
    ...
    try:
        ...
        logger.debug("mirror_mark_all_notifications_read: miroir Mongo échoué — %s", exc)


async def mirror_invoice(session, inv) -> None:
    """Miroir best-effort d'une InvoiceRecord (scalaires + line_items + flags +
    status_history) vers InvoiceDocument. Réutilisé par review.py (Lot 3) et
    invoices.py (Lot 5) — tout appelant de InvoiceRepository.save().
    """
    try:
        from src.storage.documents.invoice import InvoiceDocument
        ...
        history_rows = session.execute(
            sql_text(
                ...
                "FROM status_history WHERE invoice_id = :iid ORDER BY changed_at"
            ),
            {"iid": inv.id.hex},
        ).fetchall()
        ...
        doc: dict[str, Any] = {
            ...
            "file_mime_type": inv.file_mime_type,
            ...
            "raw_extracted_json": inv.raw_extracted_json,
            "cost_catalog_id": inv.cost_catalog_id,
            "accounting_compte": inv.accounting_compte,
            "accounting_label": inv.accounting_label,
            "charge_nature": inv.charge_nature.value if inv.charge_nature else None,
            "charge_type": inv.charge_type.value if inv.charge_type else None,
            "matched_po_id": inv.matched_po_id,
            "matched_contract_id": inv.matched_contract_id,
            "matched_client_id": inv.matched_client_id,
            "payment_term_days": inv.payment_term_days,
            "classification_reason": inv.classification_reason,
            "classification_pass": inv.classification_pass,
            "human_review_required": inv.human_review_required,
            "human_review_notes": inv.human_review_notes,
            "reviewed_by": inv.reviewed_by,
            "reviewed_at": inv.reviewed_at,
            "received_at": inv.received_at,
            "extracted_at": inv.extracted_at,
            "classified_at": inv.classified_at,
            "validated_at": inv.validated_at,
            "exported_at": inv.exported_at,
            "export_reference": inv.export_reference,
            "paid_at": inv.paid_at,
        ...
        doc["history"] = [
            {
                "id": str(uuid4()), "from_status": h[1], "to_status": h[2],
                "changed_at": _dt_or_now(h[3]), "changed_by": h[4] or "agent", "notes": h[5],
            }
            for h in history_rows
        ]

        coll = InvoiceDocument.get_pymongo_collection()
        await coll.replace_one({"_id": inv_id}, doc, upsert=True)
    except Exception as exc:
        logger.debug("mirror_invoice: miroir Mongo échoué — %s", exc)


# ── Lot 4 écriture : capex / projet_budget / budget ──────────────────────────

async def mirror_asset(asset) -> None:
    from src.storage.documents.asset import AssetDocument

    await _mirror_upsert(AssetDocument, {
        "_id": str(asset.id),
...
async def mirror_budget_line(lb) -> None:
    from src.storage.documents.ligne_budget import LigneBudgetDocument

    await _mirror_upsert(LigneBudgetDocument, {
        "_id": str(lb.id),
        "projet_id": lb.projet_id,
        "categorie": lb.categorie,
        "montant_prevu": lb.montant_prevu,
        "montant_consomme": lb.montant_consomme,
        "devise": lb.devise,
        "created_at": lb.created_at,
    })


async def mirror_delete_budget_line(ligne_id: str) -> None:
    try:
        from src.storage.documents.ligne_budget import LigneBudgetDocument

        await _mirror_delete(LigneBudgetDocument, str(UUID(ligne_id)))
    except Exception as exc:
        logger.debug("mirror_delete_budget_line: miroir Mongo échoué — %s", exc)


async def mirror_budget_plan_entry(row) -> None:
    from src.storage.documents.budget_plan import BudgetPlanDocument

    try:
        coll = BudgetPlanDocument.get_pymongo_collection()
        await coll.update_one(
            {"catalog_id": row.catalog_id, "year": row.year},
            {"$set": {
                "label": row.label, "monthly": row.monthly, "note": row.note,
                "updated_at": row.updated_at or datetime.now(timezone.utc),
            },
        ...
        logger.debug("mirror_budget_plan_entry: miroir Mongo échoué — %s", exc)


async def mirror_delete_budget_plan_entry(catalog_id: str, year: int) -> None:
    from src.storage.documents.budget_plan import BudgetPlanDocument

    try:
        coll = BudgetPlanDocument.get_pymongo_collection()
        await coll.delete_one({"catalog_id": catalog_id, "year": year})
    except Exception as exc:
        logger.debug("mirror_delete_budget_plan_entry: miroir Mongo échoué — %s", exc)


# ── Lot 5 écriture : billing / payments ───────────────────────────────────────

async def mirror_client_invoice(invoice) -> None:
    from src.storage.documents.client_invoice import ClientInvoiceDocument

    await _mirror_upsert(ClientInvoiceDocument, {
        "_id": str(invoice.id),
        "invoice_number": invoice.invoice_number,
        "issuer_name": invoice.issuer_name,
        "issuer_tax_id": invoice.issuer_tax_id,
        "issuer_address": invoice.issuer_address or "",
...
async def mirror_installment_paid(installment_id: str, paid_amount: float, paid_date: str) -> None:
    from src.storage.documents.payment_installment import PaymentInstallmentDocument

    try:
        coll = PaymentInstallmentDocument.get_pymongo_collection()
        await coll.update_one(
            {"_id": str(UUID(installment_id))},
            {"$set": {
                "status": "PAID",
        ...
        logger.debug("mirror_installment_paid: miroir Mongo échoué — %s", exc)


# ── Lot 6 écriture : admin / users / security / auth ─────────────────────────

async def mirror_user(user) -> None:
    from src.storage.documents.user import UserDocument

    await _mirror_upsert(UserDocument, {
        "_id": str(user.id),
        "nom": user.nom,
        "prenom": user.prenom,
        "email": user.email,
        ...
        "role": user.role,
        "departement": user.departement or "",
...
async def mirror_delete_user(user_id: str) -> None:
    try:
        from src.storage.documents.user import UserDocument

        await _mirror_delete(UserDocument, str(UUID(user_id)))
    except Exception as exc:
        logger.debug("mirror_delete_user: miroir Mongo échoué — %s", exc)


async def mirror_active_token(jti: str, user_id: str, expires_at, ip_address, user_agent) -> None:
    from src.storage.documents.active_token import ActiveTokenDocument

    await _mirror_upsert(ActiveTokenDocument, {
        "_id": jti,
        "user_id": str(user_id),
        "created_at": datetime.now(timezone.utc),
        "expires_at": expires_at,
...
async def mirror_token_revoked(jti: str) -> None:
    from src.storage.documents.active_token import ActiveTokenDocument
...
async def mirror_revoked_token(jti: str, reason: str, user_id: str | None) -> None:
    ...
    await _mirror_upsert(RevokedTokenDocument, {
        "_id": jti,
